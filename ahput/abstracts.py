...
from .utils import get_images_model_path
...
class BlogAbstract(models.Model):
    ...
    header_iamge = models.ForeignKey(get_images_model_path(), verbose_name=_('Header Image'), null=True, blank=True,
...
class EntryAbstract(models.Model):
    ...
    tags = ClusterTaggableManager(through='ahput.TagEntryPage', blank=True)
    date = models.DateTimeField(verbose_name=_("Post date"), default=datetime.datetime.today)
    header_image = models.ForeignKey(get_images_model_path(),
    ...
    categories = models.ManyToManyField('ahput.Category', through='ahput.CategoryEntryPage', blank=True)
